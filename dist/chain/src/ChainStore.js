...
var _require = require("graphenejs-ws");

var Apis = _require.Apis;

var _require2 = require("./ChainTypes");

var object_type = _require2.object_type;
var impl_object_type = _require2.impl_object_type;
...
var ChainStore = function () {
   ...
   _createClass(ChainStore, [{
         ...
         if (ChainValidation.is_account_name(id_or_account, true) || id_or_account.substring(0, 4) == "1.2.") {
            ...
            if (!_account2) {
               this.lookupAccountByName(id_or_account).then(function (account) {
                  ...
                  if (ChainValidation.is_object_id(witness_id)) return _this7.getObject(witness_id, on_update);
               ...
               var _ret = function () {
                  var account_id = _account2.get('id');
                  var witness_id = _this7.witness_by_account_id.get(account_id);
                  if (ChainValidation.is_object_id(witness_id)) return {
                        v: _this7.getObject(witness_id, on_update)
                     };
      ...
      value: function getCommitteeMember(id_or_account) {
         ...
         var on_update = arguments.length <= 1 || arguments[1] === undefined ? null : arguments[1];
         ...
         if (ChainValidation.is_account_name(id_or_account, true) || id_or_account.substring(0, 4) == "1.2.") {
            ...
            if (!account) {
               this.lookupAccountByName(id_or_account).then(function (account) {
                  ...
                  if (ChainValidation.is_object_id(committee_id)) return _this8.getObject(committee_id, on_update);
               ...
               var _ret2 = function () {
                  var account_id = account.get('id');
                  var committee_id = _this8.committee_by_account_id.get(account_id);
                  if (ChainValidation.is_object_id(committee_id)) return {
                        v: _this8.getObject(committee_id, on_update)
                     };
      ...
      value: function fetchFullAccount(name_or_id) {
         ...
         if (!this.fetching_get_full_accounts.has(name_or_id) || Date.now() - this.fetching_get_full_accounts.get(name_or_id) > 5000) {
            ...
            Apis.instance().db_api().exec("get_full_accounts", [[name_or_id], true]).then(function (results) {
               ...
               var vesting_balances = full_account.vesting_balances;
               var statistics = full_account.statistics;
               var call_orders = full_account.call_orders;
               var limit_orders = full_account.limit_orders;
               var referrer_name = full_account.referrer_name;
               var registrar_name = full_account.registrar_name;
               var lifetime_referrer_name = full_account.lifetime_referrer_name;
               var votes = full_account.votes;
               var proposals = full_account.proposals;
               ...
               account.orders = account.orders.withMutations(function (set) {
                  limit_orders.forEach(function (order) {
                     _this11._updateObject(order, false);
                     set.add(order.id);
                  });
               });
               ...
               account.call_orders = account.call_orders.withMutations(function (set) {
                  call_orders.forEach(function (co) {
                     _this11._updateObject(co, false);
                     set.add(co.id);
                  });
               });
               ...
               account.proposals = account.proposals.withMutations(function (set) {
                  proposals.forEach(function (p) {
                     _this11._updateObject(p, false);
                     set.add(p.id);
                  });
               });

               _this11._updateObject(statistics, false);
               var updated_account = _this11._updateObject(account, false);
               _this11.fetchRecentHistory(updated_account);
               _this11.notifySubscribers();
            }, function (error) {
               console.log("Error: ", error);
               if (ChainValidation.is_object_id(name_or_id)) _this11.objects_by_id = _this11.objects_by_id.delete(name_or_id);else _this11.accounts_by_name = _this11.accounts_by_name.delete(name_or_id);
            });
         }
         return undefined;
      }
         ...
         approvals.forEach(function (id) {
            ...
            if (impactedAccount) {
               ...
               if (!proposals.includes(objectId)) {
                  ...
                  _this14._updateObject(impactedAccount.toJS(), false);
               }
            }
         });
      }
...
function FetchChainObjects(method, object_ids, timeout) {
   ...
   return new Promise(function (resolve, reject) {
      ...
      function onUpdate() {
         var not_subscribed_yet = arguments.length <= 0 || arguments[0] === undefined ? false : arguments[0];
...
function FetchChain(methodName, objectIds) {
   var timeout = arguments.length <= 2 || arguments[2] === undefined ? 1900 : arguments[2];
